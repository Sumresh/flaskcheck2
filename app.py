...
class FilterLocationIssues(Resource):
    ...
    def post(self):
        ...
        #     issue_data = issue
            ...
            distance = self.calculate_distance((data["latitude"], data["logitude"]), issue_location)
            if distance <= 0.5:  # 500 meters in kilometers
                filtered_issues.append(issue_ids[issue])
...
#         body = request.json
#         isSaved = self.save_data(body["user_id"], {"title": body["title"]})
#         if isSaved:
#             return json.dumps({"statusCode":200, "body": "Data saved successfully."})
#         else:
#             return json.dumps({"statusCode":200, "body": "po da po."})
            
#     def save_data(self, user_id, data):
#         while True:
#             subfolder_name = str(generate_random_number())
#             path = f"{user_id}/{subfolder_name}"
#             if db.child(path).get().val() is None:
#                 break
...
# api.add_resource(SaveUserIssue, '/save_user_issue')
